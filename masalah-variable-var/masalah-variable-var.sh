Sebelumnya telah dijelaskan bahwa pengunaan var sudah tidak direkomendasikan lagi dalam bahasa pemrograman Java Script, dan diganti dengan menggunakan 'let' untuk pembuatan variable (pada pembahasan variable)
Kenapa?
Alasan kenapa var tidak direkomendasikan lagi adalah karena var tidak memiliki block scope, yang artinya dari desain awal Java Script, var ini tidak mengikat ke local scope yang telah dibahas sebelumnya (Pada pembahasan scope)
Karena masalah tersebut membuat var bisa tidak terprediksi
Contoh perbandingan antara menggunakan var dan let
